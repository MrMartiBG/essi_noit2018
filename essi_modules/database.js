var db  = function(connection){
	...
	this.fetch_car = function fetch_car(owner_id, func){
		connection.query("SELECT * FROM car WHERE owner_id = '" + owner_id + "'", func);
	}

	this.add_car_info = function add_car_info(car_info, func){
		connection.query('INSERT INTO car_info SET ?',car_info, func);
	}
	this.fetch_car_info = function fetch_car_info(car_id, func){
		connection.query("SELECT * FROM car_info WHERE car_id = '" + car_id + "'", func);
	}
